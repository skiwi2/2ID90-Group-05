 ...
 */
public class KingCountingComponent implements HeuristicComponent {
    @Override
    public int calculateComponentValue(final DraughtsState draughtsState, final boolean isWhitePlayer, final int depth, final int currentHeuristicValue) {
        ...
        if (isWhitePlayer) {
            return ((whiteKings * 30) + (whitePieces * 10) - (blackKings * 30) - (blackPieces * 10));
        }
        else {
            return ((blackKings * 30) + (blackPieces * 10) - (whiteKings * 30) - (whitePieces * 10));
        }
    }
}
